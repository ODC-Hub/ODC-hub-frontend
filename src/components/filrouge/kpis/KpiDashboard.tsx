...
export function KpiDashboard({ project, sprints, kpis }: KpiDashboardProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="flex flex-col items-center">
            ...
            <div className="text-sm text-gray-600 mt-3">Overall Progress</div>
          </div>
        </div>

        {/* Active Sprints */}
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="text-sm text-gray-600 mb-2">Active Sprints</div>
        ...
        {/* Velocity Trend */}
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="text-sm text-gray-600 mb-2">Velocity Trend</div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div className="flex items-center gap-2 mb-2">
            <div className="text-4xl font-bold text-gray-900">{Math.round(avgRiskScore)}</div>
            <AlertTriangle className={`w-6 h-6 ${riskLevel === 'high' ? 'text-red-600' :
              riskLevel === 'medium' ? 'text-orange-600' : 'text-green-600'
              }`} />
          </div>
          <div className={`inline-block px-2 py-1 text-xs font-medium rounded ${riskLevel === 'high' ? 'bg-red-100 text-red-700' :
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        <h3 className="text-lg font-bold text-gray-900 mb-4">Sprint Velocity</h3>
              ...
              <div key={index} className="flex-1 flex flex-col items-center gap-2">
                <div className="w-full flex items-end justify-center gap-2 h-48">
                  <div className="flex flex-col items-center flex-1">
                    <div
                      className="w-full bg-orange-200 rounded-t transition-all duration-500"
                      style={{ height: `${plannedHeight}%` }}
                    />
                    <span className="text-[10px] text-gray-500 mt-1">{sprint.planned}</span>
                  </div>
                  <div className="flex flex-col items-center flex-1">
                    <div
                      className={`w-full ${sprint.status === 'ACTIVE' ? 'bg-green-200' : 'bg-green-500'} rounded-t transition-all duration-500`}
                      style={{ height: `${completedHeight}%` }}
                    />
                    <span className="text-[10px] text-gray-500 mt-1">{sprint.completed}</span>
                  </div>
                </div>
                <div className="text-[10px] text-gray-700 font-medium whitespace-nowrap overflow-hidden text-ellipsis w-full text-center" title={sprint.name}>
                  {sprint.name}
                </div>
              </div>
            );
        ...
        <div className="flex items-center justify-center gap-6 mt-4 pt-4 border-t border-gray-200">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-600">Planned Effort</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-4 h-4 bg-green-500 rounded"></div>
            <span className="text-sm text-gray-600">Completed Effort</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div key={risk.category} className="text-center p-4 bg-gray-50 rounded-lg">
              <div className="text-2xl font-bold text-gray-900 mb-1">{risk.value}</div>
      ...
      {/* Sprint List */}
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        <h3 className="text-lg font-bold text-gray-900 mb-4">Sprint History</h3>
        <div className="space-y-3">
          {sprints.map((sprint: Sprint) => (
            <div key={sprint.id} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:border-orange-200 transition-colors">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  <h4 className="font-medium text-gray-900">{sprint.name}</h4>
                  <span className={`px-2 py-1 text-xs font-medium rounded ${sprint.status === 'ACTIVE' ? 'bg-blue-100 text-blue-700' :
                    sprint.status === 'CLOSED' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'
                    }`}>
                    {sprint.status}
                  </span>
                </div>
