                ...
                <div className="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none transition-all h-24 resize-none"
                    ...
                    <div>
                        ...
                        <div className="max-h-60 overflow-y-auto border border-gray-300 rounded-lg p-3 dark:border-gray-700 custom-scrollbar">
                            {users.map(u => (
                                <div key={u.id} className="flex items-center mb-2 last:mb-0 hover:bg-gray-50 p-2 rounded-md transition-colors">
                                    <input
                                        type="checkbox"
                                        id={`user-${u.id}`}
                                    ...
                                    />
                                    <label htmlFor={`user-${u.id}`} className="ml-3 text-sm text-gray-900 cursor-pointer flex-1 select-none flex items-center gap-2">
                                        {/* Avatar Logic */}
                                        {u.avatarFileId ? (
                                            <img
                                                src={`http://localhost:8080/api/users/avatar/${u.avatarFileId}`}
                                                alt={u.fullName}
                                        ...
                                        {!u.avatarFileId && (
                                            <div className="w-8 h-8 rounded-full bg-orange-100 flex items-center justify-center text-orange-600 font-bold text-[10px] uppercase shrink-0">
                                        ...
                                        <div className="flex flex-col">
                                            ...
                                            {u.fullName && <span className="text-xs text-gray-500">{u.email}</span>}
                                        </div>
                                    </label>
                                </div>
                            ))}
