...
export function ProjectOverview({ project, sprints, kpis }: ProjectOverviewProps) {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
                    <div className="flex flex-col items-center">
                        ...
                        <h3 className="text-sm text-gray-600 dark:text-gray-400 mt-4 font-medium">Progress</h3>
                    </div>
                </div>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
                    <div className="flex flex-col items-center justify-center h-full">
                        <div className="text-5xl font-bold text-gray-900 dark:text-white mb-2">{activeSprints.length}</div>
                        <h3 className="text-sm text-gray-600 dark:text-gray-400 font-medium mb-1">Active Sprints</h3>
                        <p className="text-xs text-gray-500 dark:text-gray-500">{completedSprints.length} completed</p>
                    </div>
                </div>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
                    <div className="flex flex-col items-center justify-center h-full">
                        <div className="text-5xl font-bold text-gray-900 dark:text-white mb-2">{project.members?.length || 0}</div>
                        <h3 className="text-sm text-gray-600 dark:text-gray-400 font-medium mb-1">Team Members</h3>
                    ...
                    <div className="space-y-4">
                        {recentActivities.map((activity) => (
                            <div key={activity.id} className="flex items-start gap-3 pb-4 border-b border-gray-100 dark:border-gray-700 last:border-0 last:pb-0">
                                <div className={`p-2 rounded-lg ${activity.type === 'sprint-start' ? 'bg-green-100 dark:bg-green-900/30' : 'bg-blue-100 dark:bg-blue-900/30'}`}>
            ...
            {sprints.length > 0 && (
                <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
                    <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-4">Sprint Summary</h2>
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div className="text-center p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                            <div className="text-2xl font-bold text-gray-900 dark:text-white">{sprints.length}</div>
                            <div className="text-xs text-gray-600 dark:text-gray-400 mt-1">Total Sprints</div>
                        </div>
                        ...
                        <div className="text-center p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                            <div className="text-2xl font-bold text-blue-600">{sprints.filter(s => s.status === 'PLANNED').length}</div>
                            <div className="text-xs text-gray-600 dark:text-gray-400 mt-1">Planned</div>
                        </div>
                    </div>
                </div>
            )}
